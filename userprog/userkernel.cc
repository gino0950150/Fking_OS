...
void
UserProgKernel::swapOut(int victim)
{
	for(int i = 1; i <= execfileNum; i++) 
		if(t[i]->space->swapOut(victim, t[i]->getName()))
			break;
}

void
UserProgKernel::swapIn(int victim)
{
	// cout << "UserProgKernel ReadRegister(BadVAddrReg) = " << machine->ReadRegister(BadVAddrReg) << endl;
	currentThread->space->swapIn(victim, machine->ReadRegister(BadVAddrReg));
}
